...
class wide_VAE(nn.Module):
    def __init__(self, featureDim = 128*8*8, zDim = 512, dropRate = 0.0):
        ...
        self.enc_block1 = self._make_layer_encoder(3, in_planes=16, out_planes=64, stride=2)
        self.enc_block2 = self._make_layer_encoder(3, in_planes=64, out_planes=128, stride=2)
        self.norm = nn.BatchNorm2d(128)
        self.encFC1 = nn.Linear(featureDim, zDim)
        self.encFC2 = nn.Linear(featureDim, zDim)

        self.decFC1 = nn.Linear(zDim, featureDim)
        self.dec_block1 = self._make_layer_decoder(3, in_planes=128, out_planes=64 ,stride=2)
        self.dec_block2 = self._make_layer_decoder(3, in_planes=64, out_planes=16, stride=2)
...
class classifier(nn.Module):
    def __init__(self, input_dim = 128, feature_dim=10):
        ...
        self.block2 = self._make_layer(3, int(input_dim*2), int(input_dim*3), 1)
        self.block3 = self._make_layer(3, int(input_dim*3), self.out_dim, 2)
