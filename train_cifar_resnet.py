...
def train(vae_model, c_model, data_loader, vae_optimizer, c_optimizer, epoch_num):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        x_hat, mean, log_v, z = vae_model(data)
...
def test(vae_model, c_model, lr=0.01, num=10):
    ...
    for data, target in test_loader:
        ...
        _,_,_,z = vae_model(data)
        logit = c_model(z)
        err_nat += (logit.data.max(1)[1] != target.data).float().sum()
        logit_new = testtime_update_cifar_opt(vae_model, c_model, data, target,learning_rate=lr, num=num, opti='adam', nc=0)
        ...
        _,_,_,z = vae_model(x_adv)
        logit_adv = c_model(z)
        logit_adv_new = testtime_update_cifar_opt(vae_model, c_model, x_adv, target,learning_rate=lr, num=num, opti='adam', nc=0)
...
def main():
    dim = 256
    vae_model = res_VAE(zDim=dim).to(device)
    vae_optimizer = optim.Adam(vae_model.parameters(), lr=args.lr)
    c_model = classifier(input_dim=dim).to(device)
    ...
    if args.test_num == 0:
        ...
        for epoch in range(1, args.epochs+1):
            ...
            #     c_optimizer = optim.Adam(c_para, lr=args.lr)
        ...
        test(vae_model, c_model, lr=0.001, num=70)
